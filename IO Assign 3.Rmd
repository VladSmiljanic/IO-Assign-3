...
&=\frac{p_k}{s_j}(-\alpha) s_j s_k (ND)^{-1}\sum^{ND}_i \\
&=\frac{p_k}{s_j}(-\alpha) s_j s_k (ND)^{-1}ND \\
&=-\alpha s_k p_k\\
\end{align*}

This ends up becoming the the logit cross-price elasticity.

\newpage

For own price elasticity:
\begin{align*}
\frac{\partial s_j}{\partial p_j}  &=  (ND)^{-1}\sum^{ND}_i  - \alpha \frac{(\exp(\delta_{jt} +\sigma_\beta v_\beta x_j )}{(1 + \sum^J_k \exp(\delta_{kt} + x_{kt}\sigma_{\beta}v_{it}))} \\
\frac{\partial s_j}{\partial p_j} &= (ND)^{-1}\sum^{ND}_i  - \alpha s_j\\
\frac{\partial s_j}{\partial p_j} &= - \alpha s_j\\
\end{align*}

For elasticity multiply by $\frac{p_k}{s_j}$:

\begin{align*}
\frac{\partial s_j}{\partial p_j} \frac{p_j}{s_j} &= - \alpha s_j \frac{p_j}{s_j} \\
&=-\alpha p_j\\
\end{align*}
