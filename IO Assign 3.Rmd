...
Since in the random coefficients model the $\beta$ are lognormally distributed we can transform the estiamtes by taking the logs of them to get $\ln(\beta))$ that are normal distributed. We can than use the transformed variable to generate the mean and variance of $\beta$ where mean $= \exp(\textrm{logcoef} + s/2)$ and Variance 
