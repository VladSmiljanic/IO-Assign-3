...
\frac{\partial s_j}{\partial p_k}  &=  (ND)^{-1}\sum^{ND}_i  - \alpha \frac{(\exp(\delta _jt +\sigma_\beta v_\beta x_j )*(\exp(\delta_{kt} + x_{kt}\sigma _{\beta}v_{it})}{(1 + \sum^J_k \exp(\delta_{kt} + x_{kt}\sigma _{\beta}v_{it}))^2 } \\
\frac{\partial s_j}{\partial p_k} &= (ND)^{-1}\sum^{ND}_i  - \alpha s_j \frac{(\exp(\delta_{kt} + x_{kt}\sigma _{\beta}v_{it})}{(1 + \sum^J_k \exp(\delta_{kt} + x_{kt}\sigma _{\beta}v_{it})) }\\
...
Since this is using the approximation to the market shares this is approximately the cross price elasticity
