...
Logit Specification  w/ endogeneity taken into account and Nested Logit specification w/ endogeneity taken into account

Note that Ford = 5 ,VW = 26 and Germany = 3 and we are running this code after Q2 so already have generated instruments for logit and nested logit.

```{}
#delimit ;
mergersim init, nests(segment) price(price) quantity(qu) marketsize(MSIZE) firm(firm);

xtivreg M_ls (price = 	i_horsepower i_fuel i_width i_height i_domestic i_num
			if_horsepower if_fuel if_width if_height if_domestic i_num)
			horsepower fuel width height domestic year country2-country5, fe;
